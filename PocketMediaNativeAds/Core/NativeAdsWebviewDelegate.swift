...
public class NativeAdsWebviewDelegate: NSObject, UIWebViewDelegate {
	...
	private func checkSimulatorURL(_ url: URL) -> URL {
		#if DEBUG
	...
	public func checkIfAppStoreUrl(_ request: URLRequest) -> Bool {
		...
		Logger.debugf("%s Returned false", request.url!.absoluteString)
		return false
	}
     ...
     */
	public func openSystemBrowser(_ url: URL) {
	...
	private func createLoadingIndicator(_ parentView: UIView) {
		// Box config:
