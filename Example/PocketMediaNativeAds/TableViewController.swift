...
class TableViewController: UITableViewController, NativeAdConnectionProtocol {
  ...
  func loadLocalJSON(){
    if let path = NSBundle.mainBundle().pathForResource(Constants.DummyFile, ofType: "json"), jsonData = NSData(contentsOfFile: path), json: NSArray = NSJSONSerialization.JSONObjectWithData(jsonData, options: NSJSONReadingOptions.MutableContainers, error: nil) as? NSArray {
